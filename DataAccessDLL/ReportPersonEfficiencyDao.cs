...
namespace DataAccessDLL
{
    ...
    {
        public DataTable GetPersonEfficiency(string PID, DateTime StartDate, DateTime Enddate, int FinishStatus)
        {
            ...
            '日常' as source, r.name as name,r.Desc,date(r.startdate) as StartDate,date(r.enddate) as enddate,rw.workload,rw                                .actualworkload ,'1' as type,s.name as allname,
            ...
            '交付物' as source,d.name as name,d.Desc,date(d.StartDate) as startdate,date(d.enddate) as enddate,dw.workload,dw.actualworkload ,
            ...
            '问题' as source,t.name as name ,t.Desc,date(t.StartDate) as startdate,date(t.enddate) as enddate,tw.workload,tw.actualworkload,'2'             as type ,s.name as allname,
            ...
            //date(StartDate)>=date(@staretdate) 
            ...
            union
            /*标题*/
            select name as RowNo,'来源' as source,'名称' as name ,'描述' as desc,'开始' as StartDate,'结束' as enddate,'工作量' as workload,                '实际工作量' as actualworkload ,0 as type,name as allname,'效率系数' as efficiency,null as finishstatus from stakeholders s
            where s.pid=@pid and s.status =@status
            union

            /*签字行*/
            select null as RowNo,null as source,null as name ,null as desc,null as StartDate,null as enddate,null as workload,'平均系数:' as                       actualworkload ,-3 as type,name || '1' as allname,
            (select cast(round((ifnull((select sum(workload) from troublework where s.id=manager ),0)+
            ifnull((select sum(workload) from routinework where s.id=manager ),0)+
            ifnull((select sum(workload) from deliverableswork where s.id=manager ),0)
            )*1.0/(ifnull((select sum(actualworkload) from troublework where s.id=manager ),0)+
            ifnull((select sum(actualworkload) from routinework where s.id=manager ),0)+
            ifnull((select sum(actualworkload) from deliverableswork where s.id=manager ),0))*100.0,1) as varchar(20)) ||'%' ) 
            as efficiency,
