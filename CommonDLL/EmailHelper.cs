    ...
    public class EmailHelper
    {
        ...
        public string SmtpHost
        {
            get
            {
                ...
                {
                    _SmtpHost = CommonHelper.GetConfigValue("SmtpHost");
                }
                return _SmtpHost;
            }
        }
        ...
        public int SmtpPort
        {
            get
            {
                if (_SmtpPort == -1)
                {
                    if (!int.TryParse(CommonHelper.GetConfigValue("SmtpPort"), out _SmtpPort))
                    {
                        _SmtpPort = 25;
                    }
                }
        ...
        public string FromEmailAddress
        {
            get
            {
                ...
                {
                    _FromEmailAddress = CommonHelper.GetConfigValue("FromEmailAddress");
                }
        ...
        public string FormEmailPassword
        {
            get
            {
                ...
                {
                    _FormEmailPassword = CommonHelper.GetConfigValue("FormEmailPassword");
                }
        ...
        {
            this.ToList = CheckAddr(toList);
            this.CCList = CheckAddr(ccList);
            this.BccList = CheckAddr(bccList);
            this.Subject = subject;
            this.IsBodyHtml = isBodyHtml;
            this.Body = body;
        }
        ...
        private string CheckAddr(string str)
        {
            if (string.IsNullOrEmpty(str))
                return "";
            Regex regAddr = new Regex(@"\(\w*\)");//去除括号及括号内容的正则表达式
            str = str.Replace(";", ",");
            if (regAddr.IsMatch(str))
            {
                str = regAddr.Replace(str, "");
            }
