...
{
    public partial class Report_PersonnelExecutiveEfficiency : BaseForm
    {
        #region 业务初始化
        ReportPersonEfficieBLL bll = new ReportPersonEfficieBLL();
        ...
        #endregion
        public Report_PersonnelExecutiveEfficiency()
        {
            InitializeComponent();
        ...
        private void DataBind()
        {
            ...
            dt = bll.GetPersonEfficiency(ProjectId, dtis.Value, dtie.Value, FinishStatus);
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            #region 项目计划
            List<string> columns;
            columns = new List<string>() { "RowNo", "source", "name", "desc", "StartDate", "enddate", "workload", "actualworkload","efficiency" };
            ...
            foreach (DataRow item in dt.Rows)
            {
                ...
                if (item["type"].ToString() == "0")
                {
                    ...
                    if (last > 2)
                    {
                        excel.SetCellsBorder(first, 1, last, 9);//设置边框
                    }
                    first = rowno;
                    excel.SetCellsStyle(rowno, 1, rowno, 9, Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter, true, ColorIndex.灰色25);//设置                    
                }

            }
            excel.SetCellsBorder(first, 1, dt.Rows.Count + 1, 9);//设置边框
            excel.InsertRows(1, 1);
            excel.MergeCells(1, 1, 2, 9, project.Name+"—人员执行效率");//项目名称
