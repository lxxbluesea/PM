...
{
    public partial class Report_Receivables : Common.BaseForm
    {

        #region 业务初始化
        ReportReceivablesBLL bll = new ReportReceivablesBLL();
        ...
        /// 绑定收款列表数据
        /// </summary>
        private void ReceivablesBind(List<string> pids)
        {
            dt = bll.GetReceivables(pids);
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            ExcelHelper excel = new ExcelHelper(FileHelper.GetWorkdir() + FileHelper.GetUploadPath(UploadType.Report_Receivables, "", "") + ConstHelper.Report_Receivables, saveFileName);

            #region 标题信息
            excel.SetCells(1, 1, "收款批次");
            excel.SetCells(1, 2, "完成比例");
            excel.SetCells(1, 3, "完成情况");
            excel.SetCells(1, 4, "收款金额");
            excel.SetCells(1, 5, "条件");
            excel.SetCells(1, 6, "备注");
            excel.SetCells(1, 7, "收款日期");
            ...
            #region 项目计划
            List<string> columns;
            columns = new List<string>() { "BatchNo", "Ratio", "FinishStatus", "Amount", "Condition", "Remark", "Indate" };
